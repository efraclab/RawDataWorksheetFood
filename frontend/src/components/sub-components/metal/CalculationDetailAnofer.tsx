...
const CalculationDetailAnofer: React.FC<Props> = ({
  ...
  const extractValues = (sp?: SamplePreparationMetal) => {
    if (!sp)
      return { sw1: null, v1: null, v2: null, v3: null, v4: null, v5: null };
    ...
    return {
      ...
      v3: d2?.value2 ?? null,     // 2nd Dil makeup → DF1 = V3/V2
      v4: d3?.value1 ?? null,     // 3rd Dil take (optional)
      v5: d3?.value2 ?? null,     // 3rd Dil makeup → DF2 = V5/V4 (optional)
    };
  };

  // Anofer formula: % of L.C. =
  //   (Sample − Blank) × V1 × DF1 × DF2 × AvgWeight × 100
  //   ──────────────────────────────────────────────────────
  //                  SW1 × 1000 × Label Claim
  //
  // DF1 = V3/V2 (2nd dilution), DF2 = V5/V4 (3rd dilution, optional)
  const compute = (
    instSample: string,
    ...
    const v5n = prepNum(v5);
    const df1 = v3n / v2n;
    const df2 = v5n / v4n;
    const avgN = toCanonicalGrams(parseFloat(avgWeight), avgWeightUnit);
    const lcN = parseFloat(labelClaim);
    if (!Number.isFinite(avgN) || !Number.isFinite(lcN) || lcN <= 0) {
      return null;
    }
    const numerator = (sample - blank) * v1n * df1 * df2 * avgN * 100;
    const denominator = sw1n * 1000 * lcN;
  ...
  const v5Eff = prepNum(calculation.v5);
  const df1Eff = v3Eff / v2Eff;
  const df2Eff = v5Eff / v4Eff;
  ...
  const avgNum = toCanonicalGrams(
    parseFloat(calculation.avgWeight),
    calculation.avgWeightUnit
  );
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100">
                Anofer — % of Label Claim calculation
              </p>
            </div>
          </div>
        ...
        {isExpanded && (
          ...
          >
            <div className="p-6 bg-gradient-to-b from-gray-50 to-white space-y-6">
                    ...
                    <div className="flex-1 flex flex-col items-center">
                      <div className="text-center border-b-2 border-black pb-2 mb-2 px-2 w-full">
                        ...
                        <p className="text-[10px] text-gray-500 mt-0.5">
                          DF1 = V3/V2 (2nd dil.) &nbsp;|&nbsp; DF2 = V5/V4 (3rd dil., if used)
                        </p>
                      </div>
                      <div className="text-center px-2 w-full">
                        <p className="text-xs font-mono text-black break-words">
                          Sample Weight (SW1) × 1000 × Label Claim
                        </p>
                      </div>
                    </div>
              ...
              {/* Avg. Weight & Label Claim inputs */}
              <div className="bg-gradient-to-r from-emerald-50 to-slate-50 rounded-lg p-4 border-2 border-emerald-200">
                <h5 className="text-sm font-bold text-gray-700 mb-3">
                  Average Weight &amp; Label Claim
                </h5>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="0.0"
                        className={`flex-1 min-w-0 px-3 py-2 bg-white border rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-emerald-400 ${
                          !calculation.avgWeight ? "border-amber-400" : "border-emerald-300"
                        }`}
                      />
                ...
                <div className="p-5 space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <DFChip
                      ...
                      takeLabel="V2"
                    />
                    <PrepChip label="V4 (3rd Dil. Take)" value={calculation.v4} unit="mL" />
                    <PrepChip label="V5 (3rd Dil. Makeup)" value={calculation.v5} unit="mL" />
                    <DFChip
                      label="DF2 = V5/V4"
                      makeup={calculation.v5}
                      take={calculation.v4}
                      makeupLabel="V5"
                      takeLabel="V4"
                    />
                    ...
                    <div className="flex flex-col items-center">
                      <div className="text-center border-b-2 border-black pb-2 mb-2 px-2 w-full">
                        <p className="text-xs font-mono text-black break-words">
                          ...
                          {fmtN4(v1Eff)} × {fmtN4(df1Eff)} × {fmtN4(df2Eff)} ×{" "}
                          {fmtN4(avgNum)} × 100
                        </p>
                        <p className="text-[10px] text-gray-500 mt-0.5">
                          (Sample/Blank in ppm; DF1={fmtN4(df1Eff)}, DF2={fmtN4(df2Eff)}; Avg. Weight in g)
                        </p>
                      </div>
                      <div className="text-center px-2 w-full">
                        <p className="text-xs font-mono text-black break-words">
                          {fmtN4(sw1Eff)} × 1000 × {fmtN4(lcNum)}
                        </p>
                      </div>
                    </div>
                  </div>
