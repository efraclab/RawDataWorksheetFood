...
const extractValues = (sp?: SamplePreparationMetal) => {
  const empty = { v1: null, v2: null, v3: null, v4: null, v5: null, v6: null, v7: null };
  if (!sp) return empty;
...
const CalculationDetailAasWater: React.FC<Props> = ({
  ...
  const computeResult = (
    ...
    v4: string | null, v5: string | null, v6: string | null, v7: string | null,
  ): string | null => {
  ...
  useEffect(() => {
    ...
    const newResult = computeResult(
      ...
      ex.v1, ex.v2, ex.v3, ex.v4, ex.v5, ex.v6, ex.v7,
    );
        ...
        <div className="relative flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-4 flex-1 cursor-pointer select-none" onClick={() => setIsExpanded(!isExpanded)}>
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100">AAS (Water) — Content calculation</p>
            </div>
          </div>
