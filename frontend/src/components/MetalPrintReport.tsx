...
const MetalWorksheet: React.FC<WorksheetProps> = ({
  ...
  // Tracks which parameters the analyst has explicitly clicked "Start Revision" on.
  // Until started, the parameter stays locked even in "analysis revision" status.
  const [revisionStartedParams, setRevisionStartedParams] = useState<Set<number>>(new Set());

  const handleStartRevision = (parameterId: number) => {
  ...
  const isParameterEditableForAnalyst = useCallback(
    (parameterId: number): boolean => {
      ...
      if (status === "analysis revision") {
        // Only editable once the analyst has explicitly clicked "Start Revision"
        return revisionStartedParams.has(parameterId);
      }
      return ["created", "analysis started"].includes(status);
    },
  ...
  const LockedParameterOverlay: React.FC<{ parameterId: number }> = React.memo(
    ({ parameterId }) => {
      ...
      if (role.toLowerCase() === "analyst" && isAnalysisRevision && param) {
        ...
        const isRevisionStarted = revisionStartedParams.has(parameterId);
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {isRevisionStarted ? (
                  ...
                  <motion.button
                    onClick={() => handleStartRevision(parameterId)}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
